...
from pymupdf_fonts import fitzfont  # Importar correctamente la función fitzfont para cargar fuentes
...
# Función para obtener OTM con ID_TAREA = 5, ESTADO = 5, y TIPO_OTM = 'PREDICTIVO'
def obtener_otms_con_tarea_5_y_estado_5_predictivo():
    query = f"""
    ...
    FROM {mant_otm_table} 
    WHERE ESTADO = 5 AND TIPO_OTM = 'PREDICTIVO'
    """
    with engine.connect() as conn:
...
# Función para organizar archivos en carpetas según SECTOR/Año/Mes/Semana/ID_TAREA
def crear_ruta_carpeta_por_tarea(sector, fecha_programada, id_tarea):
    # Obtener el año, mes y semana de la fecha programada
    anio = fecha_programada.year
    mes = fecha_programada.month
    semana = fecha_programada.isocalendar()[1]

    # Crear la ruta en la estructura deseada: SECTOR/Año/Mes/Semana/ID_TAREA
    ruta_carpeta = os.path.join(f"Por Tarea",
        sector,  # Añadir el sector como la primera parte del directorio
        str(anio),
        calendar.month_name[mes],
        f"Semana_{semana}",
        f"ID_TAREA_{id_tarea}-{fecha_programada.day}-{fecha_programada.month}"
    )
...
def rellenar_pdf(datos_otm, pdf_template, pdf_salida):
    ...
    page = doc[0]  # Supongamos que estamos trabajando en la primera página
      ...
      # Cargar la fuente Noto Sans Symbols Regular
    page.insert_font(fontname="figo", fontbuffer=font)
    ...
    # Obtener la fecha programada y construir la ruta de almacenamiento del PDF por ID_TAREA
    fecha_programada = datos_otm[2]
 ...
 # Definir encabezado de la tabla
    ...
    line_height = 10  # Altura entre filas
    ...
    # Verificar si 'Descripción2' es NaN, si es así, reemplazarla por una cadena vacía
        descripcion2 = row["Descripción2"] if pd.notna(row["Descripción2"]) else ""
        descripcion2_truncada = (descripcion2[:40] + '...') if len(descripcion2) > 30 else descripcion2
        ...
        tick_size = 10  # Tamaño del cuadro de selección (más pequeño para ajustarse mejor)
    ...
    for _, (x, y, valor) in campos.items():
        if valor is not None:
            page.insert_text((x, y), str(valor), fontsize=9, color=(0, 0, 0), fontname='figo')
...
# Función para unir múltiples PDFs en uno solo
def unir_pdfs(lista_pdfs, archivo_salida):
    doc_unido = fitz.open()
    for pdf in lista_pdfs:
        pdf_doc = fitz.open(pdf)
        doc_unido.insert_pdf(pdf_doc)
        pdf_doc.close()
    doc_unido.save(archivo_salida)
    doc_unido.close()


# Función principal para generar PDFs y combinarlos por ID_TAREA y semana
def generar_pdfs_otms_tarea_5_estado_5_predictivo():
    otms = obtener_otms_con_tarea_5_y_estado_5_predictivo()
    pdf_template = r"C:\Users\nmercado\Documents\Scripts\OTM\Base OTM.pdf"

    # Agrupar PDFs por ID_TAREA, Año, Mes y Semana
    otms_por_grupo = {}
    ...
    for datos_otm in otms:
        ...
        # Obtener año, mes y semana de la fecha programada
        anio = fecha_programada.year
        mes = fecha_programada.month
        semana = fecha_programada.isocalendar()[1]
    

        # Crear una clave única para agrupar por ID_TAREA, Año, Mes y Semana
        clave_grupo = (id_tarea, anio, mes, semana)

        nombre_pdf = f"OTM_{id_otm}_ID_TAREA_{id_tarea}_Semana_{semana}.pdf"
        ruta_carpeta = crear_ruta_carpeta_por_tarea(datos_otm[3], fecha_programada, id_tarea)
        pdf_salida = os.path.join(ruta_carpeta, nombre_pdf)

        # Rellenar el PDF individual
        rellenar_pdf(datos_otm, pdf_template, pdf_salida)
        print(f"PDF '{nombre_pdf}' generado en la carpeta correspondiente.")

        # Agrupar por ID_TAREA, Año, Mes y Semana
        if clave_grupo not in otms_por_grupo:
            otms_por_grupo[clave_grupo] = []
        otms_por_grupo[clave_grupo].append(pdf_salida)

    # Combinar los PDFs por ID_TAREA, Año, Mes y Semana
    for clave_grupo, lista_pdfs in otms_por_grupo.items():
        if len(lista_pdfs) > 1:
            id_tarea, anio, mes, semana = clave_grupo
            ruta_carpeta = os.path.dirname(lista_pdfs[0])
            archivo_salida = os.path.join(ruta_carpeta, f"OTM_Combined_ID_TAREA_{id_tarea}_Semana_{semana}.pdf")
            unir_pdfs(lista_pdfs, archivo_salida)
            print(f"PDF combinado generado: {archivo_salida}")



# Ejecutar el script
if __name__ == "__main__":
      generar_pdfs_otms_tarea_5_estado_5_predictivo()