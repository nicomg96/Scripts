...
# Función para organizar archivos en carpetas según SECTOR/Año/Mes/Semana/NOMBRE_MAQUINA
def crear_ruta_carpeta(sector, fecha_programada, nombre_maquina):
    ...
    # Crear la ruta en la estructura deseada: SECTOR/Año/Mes/Semana/NOMBRE_MAQUINA
...
def rellenar_pdf(datos_otm, pdf_template, pdf_salida):
    ...
    nombre_estado = estados.get(id_estado, " ")
        ...
        # Obtener la fecha programada y construir la ruta
    fecha_programada = datos_otm[2]  # FECHA_PROGRAMADA está en la columna 2
    ruta_carpeta = crear_ruta_carpeta(datos_otm[3], fecha_programada, nombre_maquina)
    pdf_salida = os.path.join(ruta_carpeta, pdf_salida)
...
# Función principal para generar PDFs para todas las OTM con ID_TAREA = 5, ESTADO = 5 y TIPO_OTM = 'PREDICTIVO'
def generar_pdfs_otms_tarea_5_estado_5_predictivo():
    ...
    for datos_otm in otms:
        id_otm = datos_otm[0]
        nombre_pdf = f"OTM_{id_otm}.pdf"  # Nombre del PDF con el número de OTM
        rellenar_pdf(datos_otm, pdf_template, nombre_pdf)
